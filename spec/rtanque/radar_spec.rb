...
describe RTanque::Bot::Radar do
  def reflection(heading, distance, name, type)
    described_class::Reflection.new(RTanque::Heading.new(heading), distance, name, type)
  end

  let(:radar) { described_class.new(mockbot(10, 10), RTanque::Heading.new(0)) }

  context '#scan' do
    context 'scanning bots' do
      ...
      it 'reflection should be correct' do
        ...
        expect(radar.first).to eq reflection(0, 10.0, 'otherbot', :bot)
      end
      ...
      it 'reflections should be correct' do
        ...
        expect(radar.to_a).to include(
          reflection(RTanque::Heading::N, 10.0, 'below', :bot),
          reflection(RTanque::Heading::N, 40.0, 'right', :bot)
        )
      end
      ...
      it 'should detect bot in same position' do
        ...
        radar.scan(bots)
        expect(radar.to_a).to include(
          reflection(RTanque::Heading::N, 0.0, 'ontop', :bot)
        )
      end
    ...
    context 'scanning projectiles' do
      it 'should 1 Reflection' do
        radar.scan([], [mockshell(10, 20)])
        expect(radar).to have(1).reflections
        expect(radar.first).to be_an_instance_of described_class::Reflection
      end
    end
